...
import {
  ...
  AMRAZ_AUTHTOKEN,
  APP_DOWNLOAD_LINK,
  AWS_COGNITO_USER_CREATION_URL_SIT,
  FEDO_APP,
  PUBLIC_KEY,
  SALES_PARTNER_LINK,
  SALES_PARTNER_NOTIFICATION,
  TWILIO_PHONE_NUMBER,
  TWILIO_WHATSAPP_NUMBER,
  AMRAZ_SERVICEID,
  GUPSHUP_OTP_MESSAGE_FORMAT,
  GUPSHUP_OTP_VERIFICATION,
} from 'src/constants';
...
import {
  AccountZwitchResponseBody,
  createPaid,
  DateDTO,
  fetchDues,
  fetchmonths,
  MobileDtO,
  MobileNumberAndOtpDtO,
  MobileNumberDtO,
  ParamDto,
  requestDto,
  sendEmailOnCreationOfDirectSalesPartner,
  sendEmailOnIncorrectBankDetailsDto,
  User,
  YearMonthDto,
...
export class AdminService {
  constructor(
    @DatabaseTable('sales_commission_junction')
    private readonly salesJunctionDb: DatabaseService<CreateSalesJunction>,
    @DatabaseTable('sales_partner')
    private readonly salesDb: DatabaseService<CreateSalesPartner>,
    @DatabaseTable('sales_partner_requests')
    private readonly salesPartnerRequestDb: DatabaseService<CreateSalesPartnerRequest>,
    @DatabaseTable('sales_user_junction')
    private readonly salesuser: DatabaseService<SalesUserJunction>,
    @DatabaseTable('sales_user_junction')
    private readonly salesUserJunctionDb: DatabaseService<CreateSalesJunction>,
  ...
  fetchCommissionDispersals(period: PeriodRange) {
    Logger.debug(
      `fetchCommissionDispersals()  period: [${JSON.stringify(period)}]`,
      APP,
    );

    return this.salesJunctionDb
      .fetchBetweenRange(fetchDAte(new Date(), PERIODADMIN[period.period]))
      .pipe(
        switchMap((salesJunctionDoc) =>
          this.fetchPreviousMonthCommissionDispersal(
            salesJunctionDoc,
            period,
            new Date(fetchDAte(new Date(), PERIODADMIN[period.period]).from),
          ),
        ),
      );
  }

  fetchPreviousMonthCommissionDispersal(
    createSalesJunction: CreateSalesJunction[],
    period: PeriodRange,
    date: Date,
  ) {
    Logger.debug(
      `fetchPreviousMonthCommissionDispersal() createSalesJunction: [${JSON.stringify(
        createSalesJunction,
      )}] period: [${JSON.stringify(period)}] date: [${date}]`,
      APP,
    );

    return this.salesJunctionDb
      .fetchBetweenRange(fetchDAte(date, PERIODADMIN[period.period]))
      .pipe(
        map((salesJunctionDoc) => ({
          thisMonth: createSalesJunction.reduce(
            (acc, curr) => (acc += curr.paid_amount),
            0,
          ),
          previousMonth: salesJunctionDoc.reduce(
            (acc, curr) => (acc += curr.paid_amount),
            0,
          ),
        })),
  ...
  fetchInvitationResponse(state: State) {
    Logger.debug(
      `fetchInvitationResponse() state: [${JSON.stringify(state)}]`,
      APP,
    );

    if (state.state !== 'all')
      return this.salesDb
        .find({ is_active: makeStateFormat(state) })
        .pipe(map((doc) => this.fetchSignUps(doc, state)));
  ...
  fetchSignUps(createSalesPartner: CreateSalesPartner[], state: State) {
    Logger.debug(
      `fetchSignUps() createSalesPartner: [${JSON.stringify(
        createSalesPartner,
      )}]`,
      APP,
    );
    ...
    return lastValueFrom(
      from(createSalesPartner).pipe(
        concatMap((salesPartner) =>
          this.salesuser.findByPeriod({
  ...
  fetchSalesPartner(period: Period) {
    Logger.debug(
      `fetchSalesPartner() period: [${JSON.stringify(period)}]`,
      APP,
    );
  ...
  fetchSalesPartnerCommission(
    createSalesPartner: CreateSalesPartner[],
    period: Period,
  ) {
    Logger.debug(
      `fetchSalesPartnerCommission() createSalesPartner: [${JSON.stringify(
        createSalesPartner,
      )}]`,
      APP,
    );
    ...
    return lastValueFrom(
      from(createSalesPartner).pipe(
        switchMap((salesPartner) =>
          lastValueFrom(this.fetchTotalCommission(salesPartner, period)).then(
            (doc) => {
              commission.push(doc);
            },
          ),
      ...
      ...commission.reduce((prev, current) =>
        current.totalCommission > prev.totalCommission ? current : prev,
      ),
      count: createSalesPartner.length,
    }));
  }

  fetchTotalCommission(createSalesPartner: CreateSalesPartner, period: Period) {
    Logger.debug(
      `fetchTotalCommission() CreateSalesPartner: [${JSON.stringify(
        createSalesPartner,
    ...
    return this.salesJunctionDb
      ...
      .pipe(
        concatMap((doc) =>
          this.fetchSalesPartnerSignups(doc, createSalesPartner, period),
        ),
        map((doc) => doc),
      );
  }

  fetchSalesPartnerSignups(
    createSalesJunction: CreateSalesJunction[],
    createSalesPartner: CreateSalesPartner,
    period: Period,
  ) {
    Logger.debug(
      `fetchSalesPartnerSignups() createSalesJunction: [${JSON.stringify(
        createSalesJunction,
      )}],  CreateSalesPartner: [${JSON.stringify(
        createSalesPartner,
      )}], period: [${JSON.stringify(period)}]`,
      APP,
    );
    ...
    return this.salesuser
      ...
      .pipe(
        map((doc) => ({
          totalCommission: createSalesJunction.reduce(
            (acc, curr) => (acc += curr.commission_amount),
            0,
          ),
  ...
  async fetchUser(createSalesPartner: CreateSalesPartner[]) {
    Logger.debug(
      `fetchUser() createSalesPartner: ${JSON.stringify(createSalesPartner)}`,
      APP,
    );
    ...
    return lastValueFrom(
      from(createSalesPartner).pipe(
        concatMap((saleDoc) =>
          lastValueFrom(fetchUser(saleDoc.user_id.toString()))
            .then((userDoc) =>
              this.fetchAccount(userDoc, saleDoc).then((result) => {
  ...
  async fetchAccount(userDoc: User[], saleDoc: CreateSalesPartner) {
    Logger.debug(
      `fetchAccount() userDoc: ${JSON.stringify(
        userDoc,
      )}  saleDoc: ${JSON.stringify(saleDoc)}`,
      APP,
    );
    ...
    return lastValueFrom(
      ...
      const salesJunctionDoc = await lastValueFrom(
        this.salesJunctionDb.find({ sales_code: saleDoc.sales_code }),
      ).catch((error) => {
  ...
  fetchSalesPartnerAccountDetailsBySalesCode(salesCode: string) {
    Logger.debug(
      `fetchSalesPartnerAccountDetailsBySalesCode() salesCode: ${salesCode}`,
      APP,
    );
  ...
  async fetchUserById(createSalesPartner: CreateSalesPartner[]) {
    Logger.debug(
      `fetchUserById() createSalesPartner: ${JSON.stringify(
        createSalesPartner,
      )}`,
      APP,
    );
    ...
    if (!createSalesPartner[0].user_id)
      throw new NotFoundException('HSA account not found ');
    return lastValueFrom(
      from(createSalesPartner).pipe(
        concatMap(
          async (saleDoc) =>
            await lastValueFrom(fetchUser(saleDoc.user_id.toString()))
              .then((userDoc) =>
                this.fetchAccountById(userDoc, saleDoc).then((result) => {
  ...
  async fetchAccountById(userDoc: User[], saleDoc: CreateSalesPartner) {
    Logger.debug(
      `fetchAccountById() userDoc: ${JSON.stringify(
        userDoc,
      )}  saleDoc: ${JSON.stringify(saleDoc)}`,
      APP,
    );
  ...
  sentOtpToPhoneNumber(mobileNumberDtO: MobileNumberDtO) {
    Logger.debug(
      `sentOtpToPhoneNumber() mobileNumberDtO: [${JSON.stringify(
        mobileNumberDtO,
      )}]`,
      APP,
    );
    ...
    return this.http
      .post(
        `${GUPSHUP_OTP_VERIFICATION}&phone_no=${mobileNumberDtO.phoneNumber}${GUPSHUP_OTP_MESSAGE_FORMAT}`,
      )
      .pipe(
        catchError((err) => {
          return err;
  ...
  verifyOtp(mobileNumberAndOtpDtO: MobileNumberAndOtpDtO) {
    Logger.debug(
      `verifyOtp() mobileNumberAndOtpDtO: [${JSON.stringify(
        mobileNumberAndOtpDtO,
      )}]`,
      APP,
    );
    ...
    return this.http
      .post(
        `${GUPSHUP_OTP_VERIFICATION}&phone_no=${mobileNumberAndOtpDtO.phoneNumber}&otp_code=${mobileNumberAndOtpDtO.otp}`,
      )
      .pipe(
        catchError((err) => {
          return err;
  ...
  sentFedoAppDownloadLinkToPhoneNumber(mobileNumberDtO: MobileNumberDtO) {
    Logger.debug(
      `sentFedoAppDownloadLinkToPhoneNumber() mobileNumberDtO: [${JSON.stringify(
        mobileNumberDtO,
      )}]`,
      APP,
    );
    ...
    return this.client.messages
      ...
      .then((_res) => ({
        status: `Link ${APP_DOWNLOAD_LINK}  send to  ${mobileNumberDtO.phoneNumber} number`,
      }))
  ...
  sentFedoAppDownloadLinkToWhatsappNumber(mobileNumberDtO: MobileNumberDtO) {
    Logger.debug(
      `sentFedoAppDownloadLinkToWhatsappNumber() mobileNumberDtO: [${JSON.stringify(
        mobileNumberDtO,
      )}]`,
      APP,
    );
    ...
    return this.client.messages
      ...
      .then((_res) => ({
        status: `Link ${APP_DOWNLOAD_LINK}  send to  ${mobileNumberDtO.phoneNumber} whatsapp number`,
      }))
      .catch((err) => this.onTwilioErrorResponse(err));
  }

  sentFedoAppDownloadLinkToMobileAndWhatsappNumber(
    mobileNumberDtO: MobileNumberDtO,
  ) {
    Logger.debug(
      `sentFedoAppDownloadLinkToMobileAndWhatsappNumber() mobileNumberDtO: [${JSON.stringify(
        mobileNumberDtO,
      )}]`,
      APP,
    );
    ...
    return from(
      this.sentFedoAppDownloadLinkToPhoneNumber(mobileNumberDtO),
    ).pipe(
      map((_doc) =>
        this.sentFedoAppDownloadLinkToWhatsappNumber(mobileNumberDtO),
      ),
      switchMap((doc) => of({ status: 'Download link sent' })),
    );
  }

  sendEmailOnIncorrectBankDetails(body: requestDto, param: ParamDto) {
    Logger.debug(
      `sendEmailOnIncorrectBankDetails() body: [${JSON.stringify(
        body,
    ...
    return fetchUserByMobileNumber(param.mobileNumber).pipe(
      ...
      switchMap((_doc) =>
        this.salesPartnerRequestDb.find({
      ...
      switchMap((doc) => {
        if (doc.length === 0)
          throw new NotFoundException(
            'Sales Partner Request Details not found',
          );
        else
          return this.templateService
            .sendEmailOnIncorrectBankDetailsToSupportEmail(
              ...
              <sendEmailOnIncorrectBankDetailsDto>{
                name: this.salesPartnerDetails?.name,
                message: body.message,
                request_id: doc[0].request_id,
              },
            )
            .then((_res) =>
              this.templateService.sendEmailOnIncorrectBankDetailsToHsaEmail(
                <EmailDTO>{ toAddresses: ['support@fedo.health'] },
                <sendEmailOnIncorrectBankDetailsDto>{
                  name: this.salesPartnerDetails?.name,
                  message: body.message,
                  request_id: doc[0].request_id,
  ...
  login(logindto: LoginDTO) {
    Logger.debug(
      `admin-console login() loginDTO:[${JSON.stringify(
        Object.keys(logindto),
      )}}] UserLoginDTO:[${JSON.stringify(logindto)}]`,
    );
  ...
  forgotPassword(forgotPasswordDTO: ForgotPasswordDTO) {
    ...
    return this.http
      .post(`${AWS_COGNITO_USER_CREATION_URL_SIT}/password/otp/`, passcode)
      .pipe(
        catchError((err) => {
          console.log(err);
          return this.onAWSErrorResponse(err);
  ...
  async updatePaidAmount(updateAmountdto: createPaid) {
    Logger.debug(
      `updatePaidAmount() updateAmountdto: [${JSON.stringify(
        updateAmountdto,
      )}]`,
      APP,
    );
    ...
    await lastValueFrom(
      from(updateAmountdto['data']).pipe(
        map((res) => {
          return lastValueFrom(
            this.salesJunctionDb.find({ sales_code: res.salesCode }).pipe(
              map((doc) => {
                const sort_doc = Math.max(
                  ...doc.map((user) => parseInt(user['id'].toString())),
                );
                const user_doc = doc.filter((item) => item['id'] == sort_doc);
                const finalRes = user_doc[0]?.dues;
                const dueCommission =
                  Number(finalRes) - Number(res.paid_amount);
                if (user_doc.length != 0)
                  return this.salesJunctionDb.save({
                    sales_code: user_doc[0]?.sales_code,
  ...
  sendCreateSalesPartnerLinkToPhoneNumber(mobileNumberDtO: MobileDtO) {
    Logger.debug(
      `sendCreateSalesPartnerLinkToPhoneNumber() mobileNumberDtO: [${JSON.stringify(
        mobileNumberDtO,
      )}]`,
      APP,
    );
    ...
    return this.client.messages
      .create({
        body: `Click on Link ${SALES_PARTNER_LINK}?query=${encodeURIComponent(
          this.encryptPassword_(JSON.stringify(mobileNumberDtO)),
        )}`,
        from: TWILIO_PHONE_NUMBER,
        to: mobileNumberDtO.phoneNumber,
      })
      .then((_res) => ({
        status: `Link ${SALES_PARTNER_LINK}  send to  ${mobileNumberDtO.phoneNumber} number`,
      }))
      .catch((err) => this.onTwilioErrorResponse(err));
  }
  ...
  sendCreateSalesPartnerLinkToWhatsappNumber(mobileNumberDtO: MobileDtO) {
    Logger.debug(
      `sendCreateSalesPartnerLinkToWhatsappNumber() mobileNumberDtO: [${JSON.stringify(
        mobileNumberDtO,
      )}]`,
      APP,
    );
    ...
    return this.client.messages
      ...
      .then((_res) => ({
        status: `Link ${SALES_PARTNER_LINK}  send to  ${mobileNumberDtO.phoneNumber} whatsapp number`,
      }))
      .catch((err) => this.onTwilioErrorResponse(err));
  }

  sendCreateSalesPartnerLinkToMobileAndWhatsappNumber(
    mobileNumberDtO: MobileDtO,
  ) {
    Logger.debug(
      `sendCreateSalesPartnerLinkToMobileAndWhatsappNumber() mobileNumberDtO: [${JSON.stringify(
        mobileNumberDtO,
      )}]`,
      APP,
    );
    ...
    return from(
      this.sendCreateSalesPartnerLinkToPhoneNumber(mobileNumberDtO),
    ).pipe(
      map((_doc) =>
        this.sendCreateSalesPartnerLinkToWhatsappNumber(mobileNumberDtO),
      ),
      switchMap((doc) => of({ status: 'Sales Partner link sent' })),
    );
  }
    ...
    return from(fetchmonths(yearMonthDto.year)).pipe(
      concatMap(async (month: number) => {
        return await lastValueFrom(
          this.salesJunctionDb.fetchCommissionReportByYear(
            yearMonthDto.year,
            month,
          ),
        )
          .then(async (salesJunctionDoc) => {
            ...
            const date = salesJunctionDoc.map((doc) => {
              if (doc.paid_amount > 0) return doc.created_date;
            });
            const paidOn = date.filter((res) => res);
            await this.fetchSignup(yearMonthDto.year, month - 1)
              .then((signup) => {
                if (month === fetchmonths(yearMonthDto.year)[0] && month !== 12)
                  this.fetchSignup(yearMonthDto.year, month).then((doc) =>
                    reportData.push({
                      month: month,
                      hsa_sign_up: doc,
                      dues: fetchDues(salesJunctionDoc),
                    }),
                  );
                ...
                if (month === 12) {
                  lastValueFrom(
                    ...
                    reportData.push({
                      total_paid_amount: salesJunctionDoc.reduce(
                        (next, pre) => next + pre.paid_amount,
                        0,
                      ),
                      month: 12,
                      hsa_sign_up: signup,
                      paid_on: salesJunctionDoc.filter((doc) => {
                        if (doc.paid_amount > 0) return doc;
                      })[0]?.created_date,
                    });
                  });
                }
  ...
  fetchMonthlyReport(dateDTO: DateDTO) {
    Logger.debug(
      `fetchMonthlyReport() date: [${JSON.stringify(dateDTO)}]`,
      APP,
    );
    ...
    return this.salesDb
      ...
      .pipe(
        map((salesDb) =>
          this.fetchCommissionReportforSalesPartner(salesDb, dateDTO),
        ),
      );
  }

  fetchCommissionReportforSalesPartner(
    createSalesPartner: CreateSalesPartner[],
    dateDTO: DateDTO,
  ) {
    Logger.debug(
      `fetchCommissionReportforSalesPartner() createSalesPartner: [${JSON.stringify(
        createSalesPartner,
      )}]`,
      APP,
    );
    ...
    return lastValueFrom(
      from(createSalesPartner).pipe(
        switchMap((salesDoc) => {
          return lastValueFrom(
            this.fetchSignupforPerformace(salesDoc, dateDTO),
          ).then((doc) => performance.push(doc));
        }),
      ),
    ).then((_doc) =>
      applyPerformance(
        performance,
        averageSignup(
          createSalesPartner.length,
          performance.reduce((acc, curr) => (acc += curr.signups), 0),
        ),
      ),
    );
  }

  fetchSignupforPerformace(
    createSalesPartner: CreateSalesPartner,
    dateDTO: DateDTO,
  ) {
    Logger.debug(
      `fetchSignupAndPerformace() createSalesPartner: [${JSON.stringify(
        createSalesPartner,
      )}]`,
      APP,
    );
    ...
    return this.salesJunctionDb
      ...
      .pipe(
        switchMap((salesJunctionDoc) =>
          this.fetchSignUpsforPerformance(
            createSalesPartner,
            salesJunctionDoc,
            dateDTO,
          ),
  ...
  fetchSignUpsforPerformance(
    createSalesPartner: CreateSalesPartner,
    createSalesJunction: CreateSalesJunction[],
    dateDTO: DateDTO,
  ) {
    Logger.debug(
      `fetchSignUpsforPerformance() createSalesJunction: [${JSON.stringify(
        createSalesJunction,
      )}]`,
      APP,
    );
    ...
    return this.salesuser
      ...
      .pipe(
        map((doc) =>
          makeEarningDuesFormat(
            createSalesPartner.name,
            createSalesJunction.reduce(
              (acc, curr) => (acc += curr.commission_amount),
              0,
            ),
            !createSalesJunction[createSalesJunction.length - 1]
              ? 0
              : createSalesJunction[createSalesJunction.length - 1].dues,
            doc.length,
          ),
        ),
      );
  ...
  async fetchSignup(year, month) {
    ...
    return await lastValueFrom(
      this.salesUserJunctionDb.fetchCommissionReportByYear(year, month),
    )
  ...
  sendNotificationToSalesPartnerOnMobile(mobileNumberDtO: MobileDtO) {
    ...
    return this.client.messages
      .create({
        body: `Click on Link ${SALES_PARTNER_NOTIFICATION}?query=${this.encryptPassword_(
          JSON.stringify(mobileNumberDtO),
        )}`,
        from: TWILIO_PHONE_NUMBER,
        to: mobileNumberDtO.phoneNumber,
      })
      .then((_res) => ({
        status: `Link ${SALES_PARTNER_NOTIFICATION}  send to  ${mobileNumberDtO.phoneNumber} number`,
      }))
      .catch((err) => this.onTwilioErrorResponse(err));
  }
  ...
  sendNotificationToSalesPartnerOnWhatsappNumber(mobileNumberDtO: MobileDtO) {
    ...
    return this.client.messages
      ...
      .then((_res) => ({
        status: `Link ${SALES_PARTNER_NOTIFICATION}  send to  ${mobileNumberDtO.phoneNumber} whatsapp number`,
      }))
      .catch((err) => this.onTwilioErrorResponse(err));
  }

  sendNotificationToSalesPartnerOnMobileAndWhatsappNumber(
    mobileNumberDtO: MobileDtO,
  ) {
    Logger.debug(
      `sendCreateSalesPartnerLinkToMobileAndWhatsappNumber() mobileNumberDtO: [${JSON.stringify(
    ...
    return from(
      this.sendNotificationToSalesPartnerOnMobile(mobileNumberDtO),
    ).pipe(
      map((_doc) =>
        this.sendNotificationToSalesPartnerOnWhatsappNumber(mobileNumberDtO),
      ),
      switchMap((doc) =>
        of({ status: 'Notification send on mobile and whatsapp' }),
      ),
    );
  }
}
