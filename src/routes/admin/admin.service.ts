...
export class AdminService {
  ...
  login(logindto: LoginDTO) {
    ...
    return this.http.post(`${AWS_COGNITO_USER_CREATION_URL_SIT}/token`, logindto).pipe(
      catchError(err => { return this.onAWSErrorResponse(err) }),
      map((res: AxiosResponse) => ({ jwtToken: res.data.idToken.jwtToken, refreshToken: res.data.refreshToken, accessToken: res.data.accessToken.jwtToken })))
  }
