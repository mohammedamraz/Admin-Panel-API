...
export class SalesCommissionService {
    ...
    fetchEarnigReport(yearMonthDto: YearMonthDto) {
        ...
        return from(fetchmonths((yearMonthDto.year))).pipe(
            concatMap(async (month: number) => {
                return await lastValueFrom(this.junctiondb.fetchByYear({ columnvalue: yearMonthDto.salesCode, year: yearMonthDto.year, month: month.toString() }))
                    .then(async salesJunctionDoc => {
                        ...
                        await this.fetchSignup(yearMonthDto.year, month - 1, yearMonthDto)
                            .then(signup => {
                                ...
                                if (month === 12) {
                                    lastValueFrom(this.junctiondb.fetchByYear({ columnvalue: yearMonthDto.salesCode, year: (parseInt(yearMonthDto.year) + 1).toString(), month: '1' }))
    ...
    async fetchSignup(year, month, yearMonthDto: YearMonthDto) {
        ...
        return await lastValueFrom(this.salesUser.fetchByYear({  columnvalue: yearMonthDto.salesCode, year: yearMonthDto.year, month: month }))
